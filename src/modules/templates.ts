...
async function registerNativeTemplate(
  ...
  if (appliesTo && t.applyTemplateToTagsRecommended !== false) {
    await upsertTemplateProperty(result, rootBlockId, APPLY_TEMPLATE_TO_TAGS, `#${appliesTo}`, displayName);
  }
...
export async function installNativeTemplates(result: Result): Promise<void> {
  ...
  await appendManagedBlock(result, canonical, 'db-native-template-index-v4', [
    ...
    'Each template includes registry property lines and query blocks under dashboard sections.',
    'Re-run lss: 8setup-templates to add missing property/query children on existing templates.',
  ...
  result.notes.push(
    `Installed/verified ${registered} DB templates on ${pageForCanonical(canonical)}. Templates include property lines and #Query blocks where views exist.`,
  );
}
